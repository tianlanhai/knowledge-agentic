...
class MoonshotProviderFactory(BaseAIProviderFactory):
    """
    类级注释：月之暗面提供商工厂
    设计模式：模板方法模式 - 具体实现
    职责：
        1. 创建 Moonshot LLM 实例
        2. 创建 Moonshot Embeddings 实例
        3. 定义默认模型名称
    """
    ...
    DEFAULT_EMBEDDING_MODEL = "moonshot-embedding"

    # 内部变量：Moonshot需要api_key
    ...
    def _get_llm_class(self) -> type:
        """函数级注释：获取Moonshot LLM类（使用OpenAI兼容接口）"""
        try:
    ...
    def _get_embedding_class(self) -> type:
        """函数级注释：获取Moonshot Embedding类（使用OpenAI兼容接口）"""
        try:
...
__all__ = ['MoonshotProviderFactory']
